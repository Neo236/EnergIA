...
import com.energia.client.MlClient;
import com.energia.dto.CategoriaConsumo;
import com.energia.dto.DatosRegistroAnalisis;
import com.energia.dto.TipoInmueble;
import com.energia.entity.AnalisisEnergeticoEntity;
import com.energia.exception.DatosEntradaInvalidosException;
import com.energia.exception.GlobalExceptionHandler;
import com.energia.exception.MlRespuestaInvalidaException;
import com.energia.exception.ServicioMlNoDisponibleException;
import com.energia.repository.AnalisisEnergeticoRepository;
import com.energia.security.VerificadorSonda;
import com.energia.service.AnalisisEnergeticoService;
...
class AnalisisEnergeticoControllerTest {
    ...
    void testAnalizarConsumoConCabeceraSondaValidaNoPersiste() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/analisis-energetico")
                .header("X-EnergIA-Sonda", TOKEN_SONDA_TEST)
                .contentType(MediaType.APPLICATION_JSON)
                .content(json))
                .andExpect(status().isOk())
    ...
    void testAnalizarConsumoConCabeceraSondaInvalidaSiPersiste() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/analisis-energetico")
                .header("X-EnergIA-Sonda", "no-es-el-token-correcto")
                .contentType(MediaType.APPLICATION_JSON)
                .content(json))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(idGuardado.toString()));
